...
export default function Layout({ children }) {
  ...
  const handleChatClick = () => {
    window.open("https://cdn.botpress.cloud/webchat/v2.3/shareable.html?configUrl=https://files.bpcontent.cloud/2025/04/12/14/20250412141501-0QE2INQG.json", "_blank");
  };
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-orange-50 to-white text-gray-800 font-inter">
      <aside className="w-64 bg-white p-6 shadow-xl rounded-tr-3xl rounded-br-3xl border-r border-gray-200 space-y-4 transition-all">
        ...
        <button onClick={() => router.push("/immopro")} className="block w-full text-left py-2 px-3 rounded hover:bg-orange-100 transition">🏢 immopro</button>
        <button onClick={() => router.push("/rapprochement")} className="block w-full text-left py-2 px-3 rounded hover:bg-orange-100 transition">🔗 Rapprochement</button>
        <button onClick={() => router.push("/agents")} className="block w-full text-left py-2 px-3 rounded hover:bg-orange-100 transition">🧑‍💼 Agents</button>
        ...
        <button onClick={() => router.push("/boutique")} className="block w-full text-left py-2 px-3 rounded hover:bg-orange-100 transition">🛒 Boutique</button>
        ...
        <div className="border-t border-gray-200 pt-4 mt-4">
          <button onClick={async () => {
            ...
            router.push("/login");
          }} className="block w-full text-left py-2 px-3 rounded text-red-600 hover:bg-red-100 transition">🚪 Déconnexion</button>
        </div>
      </aside>
