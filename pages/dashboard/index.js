...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Derniers biens ajoutés par l'agent
      const latest = biensList.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).slice(0, 4)
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {latestBiens.map((bien) => (
              <Link key={bien.id} href={`/biens/${bien.id}`} className="block bg-orange-50 p-3 rounded-xl border hover:shadow">
                ...
                <p className="text-xs text-gray-700 font-medium">💰 {(bien.prix_vente + (bien.honoraires || 0)).toLocaleString()} €</p>
              </Link>
            ))}
          </div>
        </div>
