...
export default function Etape2() {
  ...
  const typesChauffage = [
    "Gaz", "Électrique", "Fioul", "Bois", "Pompe à chaleur", "Solaire", "Climatisation réversible", "Réseau urbain", "Autre"
  ]
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
        ...
        <div className="col-span-2">
          ...
          <div className="flex gap-4 mb-2">
            <input value={newPiece.nom} onChange={(e) => setNewPiece({ ...newPiece, nom: e.target.value })} placeholder="Nom de la pièce" className="input w-full" />
            <input value={newPiece.surface} onChange={(e) => setNewPiece({ ...newPiece, surface: e.target.value })} placeholder="Surface en m²" className="input w-1/3" />
            <button type="button" onClick={addPiece} className="bg-orange-600 text-white px-4 rounded">➕ Ajouter</button>
          </div>
          {pieces.length > 0 && (
