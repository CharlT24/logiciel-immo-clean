...
export default function ListeUtilisateurs() {
  ...
  const getUserPhoto = (photoPath) => {
    if (!photoPath) return "/no-avatar.jpg"
    if (photoPath.startsWith("http")) return photoPath
    return `https://fkavtsofmglifzalclyn.supabase.co/storage/v1/object/public/photos/avatars/${photoPath}`
  }
    ...
    <div className="max-w-7xl mx-auto px-6 py-10 space-y-6">
      ...
      {users.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {users.map((user) => {
            const photoUrl = getUserPhoto(user.photo_url)
            return (
              <div key={user.id} className="bg-white rounded-xl shadow hover:shadow-lg transition overflow-hidden flex flex-col">
                <img
                  src={photoUrl}
                  alt={user.nom}
